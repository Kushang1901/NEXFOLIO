    ...
    return (
        <div className="p-5" style={{ minHeight: "297mm", boxSizing: "border-box", width: "100%" }}>
            ...
            {data.projects && (() => {
                ...
                if (current) projects.push(current);
                if (!projects.length) return <section className="mb-4"><h5 className="fw-bold">PROJECTS</h5><p style={{ whiteSpace: "pre-line" }}>{data.projects}</p></section>;
                return (
                    <section className="mb-4">
                        <h5 className="fw-bold">PROJECTS</h5>
                        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                            {projects.map((proj, i) => (
                                <div key={i} style={{ borderLeft: "3px solid #0d6efd", paddingLeft: "12px", paddingTop: "4px", paddingBottom: "4px", background: "#f8f9ff", borderRadius: "0 6px 6px 0" }}>
                                    <div style={{ fontWeight: "700", fontSize: "0.92rem", color: "#0d6efd", marginBottom: "4px" }}>{proj.name}</div>
                                    {proj.bullets.length > 0 && (
                                        <ul style={{ margin: 0, paddingLeft: "16px", listStyleType: "disc" }}>
                                            {proj.bullets.map((b, j) => (
                                                <li key={j} style={{ fontSize: "0.85rem", color: "#374151", lineHeight: "1.5", marginBottom: "2px" }}>{b}</li>
                                            ))}
                                        </ul>
                                    )}
                                </div>
